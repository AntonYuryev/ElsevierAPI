...
def AddSemanticRefCount(EntityPandas:pd.DataFrame, LinkToEntityName:str,LinkToEntityIDs:list):
    ...
    for Entity in Entities:
        if Entity in LazyIdDict.keys():
            ...
            ProteinQuery = OQL.GetEntitiesByProps(PropertyValues=[Entity],SearchByProperties=['Name'])
            protids = PSnx.GetObjIds(ProteinQuery)
                        
            if len(protids) == 0 :
                ProteinQuery = OQL.GetEntitiesByProps(PropertyValues=[Entity], SearchByProperties=['Alias'])
                protids = PSnx.GetObjIds(ProteinQuery)
...
if __name__ == "__main__":
    ...
    if len(args.targets) > 0:
        ...
        for LinkToEntityName in LinkToEntities:
            QueryNode = OQL.GetEntitiesByProps(PropertyValues=[LinkToEntityName],SearchByProperties=['Name','Alias'],OnlyObjectTypes=QueryObjType)
            QueryOntology = OQL.GetChildEntities(PropertyValues=[LinkToEntityName],SearchByProperties=['Name','Alias'],OnlyObjectTypes=QueryObjType)
            EntityIDs = PSnx.GetObjIds(QueryNode)
            EntityIDs.update(PSnx.GetObjIds(QueryOntology))
        ...
        print("Entities in file %s were linked to %s in %s" % (EntityListFile, LinkToEntityName, PsAPI.ExecutionTime(start_time) ))
