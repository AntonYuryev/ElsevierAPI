...
Directly inhibited targets',\n'Indirectly inhibited targets',\n'Directly activated targets',\n'Indirectly activated targets'
  ...
  def __rank_drugs4(self,targets:list[PSObject],with_effect:str,correct_by_consistency:bool):
   ...
   # print(sortdict(drug2rank, by_key=False, reverse=True, return_top=25))
  ...
  def init_drug_df(self, drugs:list[PSObject]):
      ...
      count_after_inducer_removal = len(clean_drugs)
      removed_count = count - count_after_inducer_removal
  ...
  def regulatory_rank(self):
      ...
      known_drugs = ResnetGraph.names(self.disease_inhibitors)
      ...
      #disnames = self._disease2str()
      new_ranked_df._name_ = f'Drugs for {self._disease2str()}'
  ...
  def link2disease_concepts(self,in_drugdf:df):
    ...
    kwargs = {'connect_by_rels':['Regulation'],
              ...
              'init_refstat' : False,
              'concept_name': 'Cell processess to activate in '+ self._disease2str(),
              'add_relevance_concept_column':True}
    _,drug_df,concepts = self.score_concept('processes2activate',drug_df,**kwargs)
    if concepts:
      phenotypedf_rows += concepts2rows(concepts,kwargs['concept_name'])

    kwargs['with_effects'] = ['negative']
    kwargs['concept_name'] = 'Cell processess to inhibit in '+ self._disease2str()
    kwargs['column_rank'] = drug_df.max_colrank()
    _,drug_df,concepts = self.score_concept('processes2inhibit',drug_df,**kwargs)
    kwargs.pop('column_rank') #need to remove 'column_rank' from kwargs to advance ranking automatically by score_concept
    if concepts:
      phenotypedf_rows += concepts2rows(concepts,kwargs['concept_name'])
    
    # self.params['processes'] are formed by score_target_semantics():
    kwargs['with_effects'] = ['unknown']
    kwargs['concept_name'] = 'Cell processess affected by '+ self._disease2str()
    _,drug_df,concepts = self.score_concept('processes',drug_df,**kwargs)
    if concepts:
      phenotypedf_rows += concepts2rows(concepts,kwargs['concept_name'])
    ...
    kwargs['concept_name'] = 'Clinical parameters for '+ self._disease2str()
    _,drug_df,concepts = self.score_concept('clinical_parameters',drug_df,**kwargs)
    if concepts:
      phenotypedf_rows += concepts2rows(concepts,kwargs['concept_name'])
    ...
    if 'symptoms' in self.params:
      ...
      kwargs['concept_name'] = aggravate_symptoms_col
      kwargs['column_rank'] = drug_df.max_colrank() 
      _,drug_df,_ = self.score_concept('symptoms',drug_df,**kwargs)
      kwargs.pop('column_rank') # need to remove 'column_rank' from kwargs to advance ranking automatically by score_concept
      drug_df = drug_df.dfcopy(rename2={'Relevant symptoms':'Aggravated symptoms'})

      symptoms_columns = []
      colname = 'inhibit symptoms for '+ self._disease2str()
      kwargs['concept_name'] = colname
      kwargs['with_effects'] = ['negative']
      #kwargs['column_rank'] = 1
      _,drug_df,concepts= self.score_concept('symptoms',drug_df,**kwargs)
      if concepts:
        phenotypedf_rows += concepts2rows(concepts,kwargs['concept_name'])
      ...
      print('Removing drugs aggravating symptoms:')
      for col in symptoms_columns:
        ...
        # must use -1 cutoff, 0 cutoff removes drugs that are not linked to symptoms:
    ...
    kwargs['concept_name'] = 'regulation of diseases similar to '+ self._disease2str()
    kwargs['with_effects'] = ['negative']
    _,drug_df,concepts = self.score_concept('similar_diseases',drug_df,**kwargs)
    if concepts:
      phenotypedf_rows += concepts2rows(concepts,kwargs['concept_name'])

    kwargs['add_relevance_concept_column'] = False
    kwargs.pop('with_effects')
    kwargs['concept_name'] = 'regulation of '+ self._disease2str()
    kwargs.pop('clone2retrieve','') # following steps have useful articles for Bibliography worksheet
    _,drug_df = self.score_concepts(self.input_diseases,drug_df,**kwargs)

    kwargs['concept_name'] = self._disease2str()+' clinical trials'
  ...
  def drugs4metabolites(self)->ResnetGraph:
    met_targets_names = self.params.get('metabolites2inhibit',[])
    reqname = 'Find drugs for metabolites'
    if met_targets_names:
      if self.useNeo4j():
        met_targets = self.neo4j.get_nodes('SmallMol', 'Name',met_targets_names)
        cypher, params = Cypher.select_drug_targets(met_targets,relProps={EFFECT:['negative']})
        drugs4metG = self.neo4j.fetch_graph(cypher,params,reqname)
      else:
        oql = f'SELECT Relation WHERE Effect = negative AND NeighborOf upstream (SELECT Entity WHERE Name = ({met_targets_names}))\
            AND NeighborOf downstream ({OQL.select_drugs()})'
        drugs4metG = self.process_oql(oql,reqname)

      drugs = drugs4metG.regulators()
      print(f'Found {len(drugs)} drugs inhibiting {met_targets_names}')
      return drugs4metG
    else:
      return ResnetGraph()
  ...
  def drugs4add_inhibitors4(self)->tuple[ResnetGraph,ResnetGraph]:
    '''
    updates self.drugs2targets to match "add_inhibitors4" specifications
    '''
    # "add_inhibitors4" contains targets that must be activated only post-translationally, 
    # while they must be inhibited on expresison level (SPBTN1, Na,K-ATPase)
    # rational: suppress expression of dominant-negative disease-causing mutations, while activating good allele products
    reltype2targets = dict(self.params.get("add_inhibitors4",dict()))# reltype2targets={reltype:[target names]}
    if not reltype2targets: return ResnetGraph(), ResnetGraph()

    for reltype,target_names in reltype2targets.items():
      targets2inhibit = [n for n in self.__targets__ if n.name() in target_names]
      if self.useNeo4j():
        relProps = {OBJECT_TYPE:[reltype],EFFECT:['negative','positive']}
        cypher,params = Cypher.select_drug_targets(targets2inhibit,relProps=relProps)
        expanded_targets_g = self.neo4j.fetch_graph(cypher, params)
      else:
        targets2expand_dbids = ResnetGraph.dbids(targets2inhibit)
    ...
    for rel in good_rels:
      targets = rel.targets()
      assert len(targets) == 1, 'Non-binary relation detected'
      for t in targets:
        t_name = t.name()
    ...
    bad_drugs =  expanded_targets_g.subgraph_by_relprops(['positive'],[EFFECT])._psobjs_with('SmallMol','ObjTypeName')
    if bad_drugs:
      # bad_drugs hold undesired drugs that induce targets that must be inhibitied
    ...
    if good_rels:
      # good_rels hold additional drugs that inhibit targets that must be inhibitied
      good_drugsG = ResnetGraph.from_rels(good_rels)
  ...
  def drugs_induce_symptoms(self,minref=4):
    ...
    symptom_names += self.input_disease('Name')
  ...
  def add_drugs_withno_targets(self,drugs4targets:list[PSObject]):
    # adding drugs that are known to inhibit disease but have no links to disease targets
    compounds_withno_targets = list(set(self.disease_inhibitors).difference(drugs4targets))
    # self.disease_inhibitors contains both FDA approved drugs, dietary supplements and unapproved compunds
    if compounds_withno_targets:
      reqname = f'Find drugs linked to disease that have no targets linked to disease'
      if self.useNeo4j():
        drugs_withno_targets = self.neo4j.select_drugs(compounds_withno_targets)
      else:
        ...
        drugs4diseaseG = self.iterate_oql(oql_query,ResnetGraph.urns(compounds_withno_targets),request_name=reqname)
        drugs_withno_targets = drugs4diseaseG._get_nodes()
        
    if drugs_withno_targets:
      print(f'Found additional {len(drugs_withno_targets)} drugs inhibiting "{self._disease2str()}" that have no links to its targets')
    return list(drugs4targets) + drugs_withno_targets
  ...
  def select_drugs(self,limit2drugs:set[PSObject]=set())->list[PSObject]:
      ...
      self.drugs4add_inhibitors4()
      drugs4targets = set(self.drugs2targets._psobjs_with('SmallMol','ObjTypeName'))
      drugs4targets -= self._targets() # subtracting self._targets() to remove metabolites2inhibit from drug list
      drugs4targets = self.add_drugs_withno_targets(list(drugs4targets))
      print(f'Found {len(drugs4targets)} drugs linked to {len(self._targets())} targets for ranking')
      return list(drugs4targets)
  

  def drug2childdose(self,rankedf:df,multithread = True):
    print('Adding use in children from FDA drug labels')
    marketed_drugs_names = set(rankedf[rankedf[PHARMAPENDIUM_ID].notna()][PHARMAPENDIUM_ID].to_list())
    marketed_drugs_names.remove(DIETARY_SUPPLEMENT)
    if marketed_drugs_names:
      fda_api = FDA()
      return fda_api.child_doses(list(marketed_drugs_names),multithread)
    else:
      return dict()
