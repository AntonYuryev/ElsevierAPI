...
from ElsevierAPI.ResnetAPI.ZeepToNetworkx import REF_ID_TYPES,PSNetworx
...
class SearchParam:
    ...
    def __init__(self, EntityPandas:pd.DataFrame,psnx:PSNetworx,prop_names_in_header,use_cache=False,need_references=False,AnnotateNodesWith=['Name']):
        self.REL_PROPs=[]
        self.ENT_PROPs=AnnotateNodesWith
        self.__cntCache='counts_cache.tsv'
        self.__refCache='reference_cache.tsv'
        self.api_session = psnx
        self.__ReferenceViewNewColumn1 = self.api_session.mappedBy_propName
        self.__ReferenceViewNewColumn2 = 'Concept name query'
        self.__temp_id_col = 'entity_IDs'
        self.__child_ids_propName = self.api_session.child_ids_propName
        self.__col_name_prefix = "RefCount to "
        ...
        if use_cache == False:
            ...
            try: self.ReadCache()
                ...
                print ('Cannot find cache file %s.  Will have to map entities on input identifiers again!' % self.__cntCache)
                self.all_entity_ids = self.MapEntities(EntityPandas,prop_names_in_header)
    ...
    def MapEntities(self,EntityPandas:pd.DataFrame, prop_names_in_header = False):
        ...
        def get_entIDs(col_name,cell_value):
            ...
            try:
                ...
                try:
                    child_ids = [o[self.__child_ids_propName] for o in mappedPSobj]
        ...
        for propName in EntityPandas.columns:
            ...
            MappedEntitiesByProp = apply_and_concat(MappedEntitiesByProp,propName,get_entIDs,['Resnet name',PSnx.mappedBy_propName,self.__temp_id_col])
            MappedEntitiesByProp = MappedEntitiesByProp[MappedEntitiesByProp[self.__temp_id_col].notnull()]
            self.MappedEntityPandas = self.MappedEntityPandas.append(MappedEntitiesByProp)

        print ('Mapped %d out of %d identifiers to entities in database in %s\n' % (len(self.MappedEntityPandas),len(EntityPandas.index),ExecutionTime(start_mapping_time)))
        entity_id_tuples = self.MappedEntityPandas[self.__temp_id_col].tolist()
    ...
    def LinkToConcept(self,ConceptName,ConceptIDs:list,ConnectByRelTypes=[],RelEffect=[],RelDirection=''):
        ...
        NewColumnName = self.__col_name_prefix+ ConceptName
        ...
        if FoundRelations.size() > 0:
            for regulatorID, targetID, rel in FoundRelations.edges.data('relation'):
                ...
                rel[self.__ReferenceViewNewColumn1] = [entity_search_attr]
                rel[self.__ReferenceViewNewColumn2] = [ConceptName]
      
            AccumulateReference = set()
            for idx in self.MappedEntityPandas.index:
                idx_entity_ids = list(self.MappedEntityPandas.at[idx,self.__temp_id_col])
            ...
            print("Concept \"%s\" is linked to %d rows from infile by %s relations of type \"%s\" supported by %d references with effect \"%s\" in %s" % 
                (ConceptName,linked_entities_counter,FoundRelations.number_of_edges(),relTypeStr,len(AccumulateReference),effecStr,ExecutionTime(start_time)))

        else: print("Concept \"%s\" has no links to entities in infile" % (ConceptName))
        return linked_entities_counter
    

    def Print (self, refCountsOut='', referencesOut='',psnx:PSNetworx=None):  
        PandasToPrint = pd.DataFrame(self.MappedEntityPandas)    
  
        if len(refCountsOut)>0: 
            countFile = refCountsOut
            PandasToPrint = PandasToPrint.drop(self.__temp_id_col,axis=1)
        ...
        if len(referencesOut) > 0: self.need_references = True
        if self.need_references==True:
            ...
            RelPropsToPrint = [self.__ReferenceViewNewColumn1] + self.REL_PROPs + self.api_session.REF_ID_TYPES + [self.__ReferenceViewNewColumn2]
            self.api_session.PrintReferenceView(refFile,relPropNames=RelPropsToPrint,entPropNames=self.ENT_PROPs,access_mode=access_mode)
    ...
    def ReadCache(self):#returns last concept linked in cache
        try:
            ...
            return last_col_name[len(self.__col_name_prefix):]
        except FileNotFoundError:
...
if __name__ == "__main__":
    ...
    if len(args.pathways) > 0:
        ...
        print("Entities in file %s were linked to %s pathway in %s" % (EntityListFile, PathwayName, ExecutionTime(start_time)))
    else:
    ...
    if len(LinkToConcepts) > 0:
        ...
        for conceptIdx in range(0, len(LinkToConcepts.index)):
            ...
            if len(ConceptIDs) == 0: print ('No entity %s found in database' % LinkToConcept)
                ...
                print("Total %d out of %d concepts interrogated in %s" % (conceptIdx+1, len(LinkToConcepts),ExecutionTime(global_start_time)))

    print("%d entities in file %s were mapped linked to %d concepts from %s and %d concepts on cmd in %s" % (len(EntityPandas),EntityListFile,concepts_in_file,args.targets_file,concepts_in_cmd,ExecutionTime(global_start_time)))
    ...
    search_param.Print(countsOutFile,refOutFile,PSnx)
