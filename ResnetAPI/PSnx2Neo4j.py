...
class nx2neo4j:
    def __init__(self,uriNeo4j:str,userNeo4j:str,pswdNeo4j:str):
        self.NODE_REGISTRY = set()
        self.uri = uriNeo4j
        self.user = userNeo4j
        self.password = pswdNeo4j
    ...
    def load_nodes(self,resnet:PSNetworx):
        ...
        with self.driver.session() as session:
            ...
            for i, d in nodes:
                ...
                for record in result:
                    print("Neo4j got node: \"{n}\" of type {t} with URN={u}".format(n=record[0],t=record[2],u=record[1]))
    ...
    def load_relations(self,resnet:PSNetworx):
        with self.driver.session() as session:
            for regulatorID, targetID, rel in resnet.Graph.edges.data('relation'):
                result = session.write_transaction(self.__create_relation,resnet.Graph.nodes[regulatorID],resnet.Graph.nodes[targetID],rel)
                for record in result:
