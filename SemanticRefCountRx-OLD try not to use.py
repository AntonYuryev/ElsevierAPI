...
if __name__ == "__main__":
    ...
    If you want to use identifiers other than names enter appropriate Property names into search_by_properties list
    ...
    ps_api = SemanticSearch(load_api_config())
    ...
    if len(args.pathways) > 0:
        ...
        for PathwayName in LinkToPathways:
            ...
            EntityIDs.update(ps_api._get_obj_ids_by_props(EntityIDs,['id'],get_childs=True))
        ...
        print("Entities in file %s were linked to %s pathway in %s" % (EntityListFile, PathwayName, ps_api.execution_time(start_time)))


    if len(args.targets) > 0:
        start_time = time.time()
        for LinkToEntityName in LinkToEntities:
            EntityIDs = ps_api._get_obj_ids_by_props(propValues=[LinkToEntityName],search_by_properties=['Name', 'Alias'],get_childs=True)
