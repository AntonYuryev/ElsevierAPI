...
###############    ADD ADD ADD    #############################
  ...
  def __add_rel(self, rel:PSRelation,refresh_urn=False,edge_duplication=True):
    """
    ...
    if uid_pairs:
      ...
      [self.add_edge(uids[0],uids[1],
                     relation=rel,weight=rel.count_refs(),key=rel_urn) 
                     for uids in uid_pairs]   
  ...
  def add_psobj(self,node:PSObject):
      ...
      if node_uid in self.nodes():
          exist_node = self._psobj(node_uid)
  ...
  def __add_psrels(self,rels:set[PSRelation],add_nodes=True,merge=True,edge_duplication=True):
      ...
      input:
        ...
        nodes connected by relation must exist in the graph.\n
        non-directional relations are duplicated in all possible directions\n
        set add_nodes to False for speed up if nodes are already in the graph
        use ResnetGraph.from_rels() to create new graph from relations
      """
      if add_nodes:
        rel_nodes = [r.regulators()+r.targets() for r in rels]
        rel_nodes = unpack(rel_nodes)
        self.add_psobjs(rel_nodes,merge=merge)
      ...
      [self.__add_rel(rel,edge_duplication=edge_duplication) for rel in rels2add]
  

  @classmethod
  def from_rels(cls,rels:set[PSRelation]|list[PSRelation],edge_duplication=True)->"ResnetGraph":
      newG = ResnetGraph()
      rel_nodes = set()
...
################## SET SET SET ##########################################
  ...
  def add_node_annotation(self, with_new_prop:str, map2prop:str, using_map:dict[str,list]):
      """
...
############################   LABELING LABELS LABELING   ######################################
  def connectivity(self,node:PSObject,with_children=False):
    connectivity = 0
    if (nuid := node.uid()) in self:
      connectivity = self.degree(nuid)
      if with_children:
        for child in node.childs():
          if (cuid := child.uid()) in self:
            connectivity += self.degree(cuid)
    return connectivity
  ...
  def rank_regulator(self,regulator:PSObject,target_weights:dict[int,float],max_distance: int = 5) -> float:
    """
    Optimized regulator ranking using single-pass BFS.
    input:
      target_weights = {node_uid:weight}
    """
    reg_uid = regulator.uid()
    if reg_uid not in self:
        return 0.0

    regulator_rank = 0.0
    # BFS Initialization. Queue stores: (current_node_id, current_distance)
    queue = deque([(reg_uid, 0)])
    visited = {reg_uid}

    while queue:
      curr_uid, dist = queue.popleft()
      if dist >= max_distance:
          continue
          
      next_dist = dist + 1
      for neighbor in self.adj[curr_uid]:
        if neighbor not in visited:
          visited.add(neighbor)
          if neighbor in target_weights:
              weight = target_weights[neighbor]
              regulator_rank += weight / (next_dist * next_dist)
  ...
  '''
  def rank_regulatorOLD(self, regulator:PSObject, target_weights:dict, max_distance=5):
      """
      Input
      -----
  ...
  def rank_regulators(self, node_weights: dict[int, float], add2prop: str, max_distance: int = 5) -> dict:
    """
    Optimized regulator ranking using manual BFS to avoid heavy NetworkX object creation.
    """
    regulator_ranks = defaultdict(float)
    upstream_edges = set()

    for source_id, source_weight in node_weights.items():
      if source_id not in self:
        continue
          
      # Manual BFS implementation for speed:
      # Queue stores: (current_node_id, current_distance)
      queue = deque([(source_id, 0)])
      visited_for_this_source = {source_id}
      while queue:
        ...
        next_dist = dist + 1
        score_increment = source_weight / (next_dist * next_dist)
        predecessors = self.pred[curr_node]

        for predecessor in predecessors:
          upstream_edges.add((curr_node, predecessor))

          if predecessor not in visited_for_this_source:
            visited_for_this_source.add(predecessor)
            regulator_ranks[predecessor] += score_increment
            queue.append((predecessor, next_dist))

    nx.set_node_attributes(self, regulator_ranks, add2prop)

    #network adjustment boosts regulators regulating other regulators on the same level of regulator_trees:
    for regulator_id, current_rank in list(regulator_ranks.items()):
        neighborhood_weight = 0.0
        targets = self.adj[regulator_id]
        for target_id in targets:
          if (target_id, regulator_id) not in upstream_edges:
            neighborhood_weight += regulator_ranks.get(target_id, 0.0)

        if neighborhood_weight > 0:
          regulator_ranks[regulator_id] = current_rank + (0.5 * neighborhood_weight)

    nx.set_node_attributes(self, regulator_ranks, add2prop)
    #print(sortdict(regulator_ranks, by_key=False, reverse=True, return_top=25))
    return dict(regulator_ranks)

  '''
  def rank_regulatorsOLD(self,node_weights:dict[int,float],add2prop:str,max_distance=5):
      """
      Input
      -----
      ...
      for regulator_id in self.nodes():
          ...
          for target_id in my_target_ids:
              if regulator_trees.has_edge(target_id,regulator_id): # regulator_trees is reversed
...
################## SET-GET SET-GET SET-GET ###############################
  def load_references(self, 
    ...
    if not relpval2weight:
      weight_prop = None
      val2weight = None
    else:
      if len(relpval2weight) != 1:
        raise ValueError("relpval2weight must contain exactly one property key")
      weight_prop, val2weight = next(iter(relpval2weight.items()))

    graph_references = set()
    relid2refs = postgres.load_refs() if postgres else {}
    if val2weight:
      for _, _, rel in self.edges.data('relation'):
        graph_references.update(rel.refs(relid2refs=relid2refs))
        rel.set_weight2ref(weight_prop, val2weight, weight_name)
    else:
      # chain.from_iterable lazily flattens the results without building a list.
      # set.update consumes this iterator at C-speed instead of Python speed in "for" loop.
      graph_references.update(chain.from_iterable(rel.refs(relid2refs=relid2refs) 
                                    for _, _, rel in self.edges.data('relation'))
                              )
    ...
    for regulatorID, targetID, rel in my_graph.edges.data('relation'):
      ...
      for ref in rel.refs():
        ...
        try:
          ...
          if add_rel_props:
            row += [relname,reltype,releffect,refcount]
...
######################   GET GET GET   ######################################
  ...
  def ids4nodes(self, with_values=None, in_properties:list=[OBJECT_TYPE],id_type=DBID)->list[int]:
      '''
      Return
      ------
  ...
  def _psobjs_with(self,value:str|int,in_property='Name')->list[PSObject]:
      ...
      value - str,int,float
      '''
      my_uids = [uid for uid,values in self.nodes(data=in_property, default=[]) if value in values]
      return self._get_nodes(list(my_uids)) if my_uids else []


  def psobj_with_ids(self,ids:set,id_type=DBID)->list[PSObject]:
    return [PSObject(attrs) for _, attrs in self.nodes(data=True) if (nids := attrs.get(id_type)) and nids[0] in ids]


  '''
  def psobjs_withOLD(self, with_properties:list=[OBJECT_TYPE], only_with_values:list=[])->list[PSObject]:
  ...
  def psobjs_with(self, with_properties: list = [OBJECT_TYPE], only_with_values: list = []) -> list[PSObject]:
    ...
    Finds objects by Name, Alias, ObjTypeName or other list of properties.
    """
    target_values = set(only_with_values)
    if target_values:
      matching_uids = [uid for uid,attrs in self.nodes(data=True)
      if any((v := attrs.get(p)) is not None and not target_values.isdisjoint(v) for p in with_properties)]
    else:
      matching_uids = [uid for uid,attrs in self.nodes(data=True) if any(p in attrs for p in with_properties)]

    return self._get_nodes(matching_uids) if matching_uids else []
  ...
  @staticmethod
  def ids(nodes:list[PSObject]|set[PSObject],id_type=DBID)->list[int]:
    if id_type == DBID:
      return ResnetGraph.dbids(nodes)
    else:
      return ResnetGraph.urns(nodes)
  ...
  def dbid2uid(self,ids:list[int|str]=[],id_type=DBID)->dict[int,int]:
    '''
    output:
      {node.id():node.uid()}
    '''
    my_nodes = self.psobj_with_ids(set(ids), id_type=id_type)
    return {n.dbid():n.uid() for n in my_nodes}
...
################################# WRITE-DUMP, WRITE-DUMP, WRITE-DUMP ##############################
  ...
  def __2resnet(self,resnet:et._Element,ent_props:list,rel_props:list,
    ...
    if add_pathway_props:
      pathway_props = et.SubElement(resnet, 'properties',attrib=dict(), nsmap=None)
    ...
    node_attr = {'delete':'true'} if delete_nodes else dict()
...
################################# READ READ READ ##########################################
  ...
  def _parse_nodes_controls(resnet:et._Element, only_nodeprops:dict[str,list]=dict(),
    ...
    def validate(my_regulators:list[PSObject], my_targets:list[PSObject]):
      ...
      if only_nodeprops:
        all_objects = chain(my_regulators, my_targets)
        if not any(n.has_value_in(only_nodeprops) for n in all_objects):
          return False

      return True

    localid2node = dict() #[local_id] = PSObject
    for node in resnet.findall('./nodes/node',''):
      node_attrs = node.findall('attr')
      node_urn = node.get('urn')
      if node_urn in only4objs:
        node_psobj = only4objs[node_urn]
        for a in node_attrs:
          if a.get('name') != 'NodeType':
            node_psobj.update_with_value(a.get('name'), a.get('value'))
      else:
      ...
      localid2node[node.get('local_id')] = node_psobj
    ...
    if controls:
      ...
      for control_elem in controls:
        regulators = list()
        targets = list()
        for link in control_elem.findall('link'):
          link_type = link.get('type')
          if link_type == 'out': 
            targets.append(localid2node[link.get('ref')])
          else: 
        ...
        if validate(regulators,targets):
          ...
          for attr in control_elem.findall('attr'):
      ...
      return list(localid2node.values()),set()
  ...
  @classmethod
  def from_resnet(cls,resnet:et._Element,
                prop2values:dict=dict(),only_relprops:set=set(),
                only4objs:dict[str,PSObject]=dict(),on_both_ends=True,
                edge_duplication=True)->"ResnetGraph":
      '''
      input:
        prop2values={prop_name:[values]} - filter to load relations only for nodes with desired properties
      output:
  ...
  def fromRNEFflist(cls,flist:list[str],prop2values:dict=dict(),
                  only_relprops:set=set(),merge=True,edge_duplication=True)->"ResnetGraph":
      '''
      input:
        set merge=True if graph loaded form multiple RNEF files with multiple <resnet> sections
        prop2values={prop_name:[values]} - filter to load relations only for nodes with desired properties
      '''
      #max_workers = min(32,len(flist),(os.cpu_count() or 1) + 4) now is default in Python > 3.8
      combo_g = ResnetGraph()
      with ThreadPoolExecutor(thread_name_prefix='readRNEFdir') as e:
        futures = list()
  ...
  def tree4(self,root:PSObject,reverse=False,depth_limit=None):
      tree_rn = ResnetGraph()
      try:
          tree = nx.algorithms.traversal.breadth_first_search.bfs_tree(self,root.uid(),reverse,depth_limit)
...
########################  SUBGRAPH SUBGRAPH SUBGRAPH #####################################
  ...
  def regulatory_network_urn(self,for_targets_with_urns:list[str],network_name:str):
      ...
      subraph_nodes = set()
      for r,t,rel in self.iterate():
        if t.urn() in for_targets_with_urns:
          subraph_nodes.add(r)
          subraph_nodes.add(t)
          rel2copy.append(rel)
  ...
  def clean_version_number(self,in_property:str):
      uid2value = {uid: list(
                        {
                            str(i)[: str(i).rfind('.') if str(i).rfind('.') > 0 else len(i)]
                            for i in psobj.get(in_property, [])
                        }
                    )
  ...
  def make_simple(self, rel_type_rank:list[str]=[]):
    ...
    output:
      ...
      otherwise relation type is assigned according to rel_type_rank specification
    """
    # Examples:\n
    # ['DirectRegulation','Binding','ProtModification','Regulation']\n
    # ['PromoterBinding','Expression','Regulation']\n
  ...
  def subgraph_by_rels(self, rels:list[PSRelation])->'ResnetGraph':
      return ResnetGraph.from_rels(rels)
  ...
  def get_rels_between(self, reg_uids: list[int], tar_uids: list[int], 
                     by_relation_types: list[str] = None, 
                     with_effect: list[str] = None, 
                     in_direction: str = '') -> set:
    
    reg_set = set(reg_uids)
    tar_set = set(tar_uids)
    rel_types_set = set(by_relation_types) if by_relation_types else None
    effects_set = set(with_effect) if with_effect else None

    my_rels = set()
    def collect_edges(sources, targets):
      for u in sources:
        if u not in self: 
          continue 
        neighbors = self[u]
        valid_targets = targets.intersection(neighbors)
        for v in valid_targets:
          for edge_data in self[u][v].values():
            rel = edge_data.get('relation')
            if rel_types_set and rel.objtype() not in rel_types_set:
              continue
            if effects_set and rel.effect() not in effects_set:
              continue
            my_rels.add(rel)

    if not in_direction or in_direction == '>':
        collect_edges(reg_set, tar_set)
        
    if not in_direction or in_direction == '<':
        collect_edges(tar_set, reg_set)
...
############################# EMBEDDING EMBEDDING EMBEDDING #######################################
  ...
  def neo4j_dfs(self)->tuple[list[PSObject],df,df,df]:
    '''
    output:
      nodes for nodes_df,relations_df,refset_df,refs_df
    '''         
    refset_dicts = list()
    ref_dicts = list()
    my_refs = set()
    ...
    for r,t,rel in self.iterate():
      ...
      refset = {'URN':refset_id,
                ...
                "Number of references:int":len(rel.references), 
                "Number of snippets:int": rel.number_of_snippets()}
      refset_dicts.append(refset)

      rel_refs = rel.refs()
      my_refs.update(rel_refs)
      for ref in rel_refs:
        ref_id = ref.doi_or_id()
        refrel_urn = f'urn:els-BelongsTo:in-out:{ref_id}:in:{refset_id}'
        refrel_name = f'{ref_id}---BelongTo--->{relname}'
        ref2rel_dict = {':START_ID':ref_id,':END_ID':refset_id, OBJECT_TYPE:'BelongsTo','URN':refrel_urn,'Name':refrel_name}
        rel_rows.append(ref2rel_dict)
    ...
    refset_colns = ['URN','Name',OBJECT_TYPE,"Number of references:int","Number of snippets:int"]
    refset_pd = pd.DataFrame(refset_dicts,columns = refset_colns).rename(columns={OBJECT_TYPE:':LABEL','URN':':ID'})

    reldf_cols = [':START_ID',':END_ID','Ontology','Relationship', OBJECT_TYPE,'URN','Name']
    relations_pd = pd.DataFrame(rel_rows,columns=reldf_cols).rename(columns={OBJECT_TYPE:':TYPE'})

    for ref in my_refs:
      assert isinstance(ref,Reference)
      for textref,sentence in ref.sentences():
        id_type, identifier = ref.get_doc_id() 
        if id_type and textref.startswith(('info:','urn:agi-nctrial')):
          refdic = {'Identifier':id_type+':'+identifier,
                    SENTENCE:textref+' '+sentence.replace(' ( )',''),
                    PUBYEAR:ref.pubyear(),
                    JOURNAL:ref.journal(),
                    TITLE:ref.title(),
                    AUTHORS: ';'.join(ref.author_list())
                  }
          ref_dicts.append(refdic)
    return self._get_nodes(),df.from_pd(relations_pd),df.from_pd(refset_pd),df(ref_dicts)
