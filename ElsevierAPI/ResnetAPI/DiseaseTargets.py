...
class DiseaseTargets(SemanticSearch):
    ...
    def __init__(self,*args,**kwargs):
        ...
        # targets4strictmode has PSObjects for targets directly linked to disease or with GVs linked to disease or from disease pathways
        ...
        # cache_networks:
        self.protein2disease = ResnetGraph() # includes GVs
        self.ppi  = ResnetGraph() # protein-protein physical interactions (Binding,DirectRegulation,ProteModification) to find partners
        self.ppmet = ResnetGraph() # protein-metabolite regulation network (MolTransport,MolSynthesis,ChemicalReaction) to find partners
        self.ppprocess = ResnetGraph() # protein-CellProcess regulation network (Regulation) to find partners
        self.drug_effects = ResnetGraph() # drug-disease regulation network (Regulation)
        self.drugs2targets = ResnetGraph() # minimumal graph for drug ranking calculation made by self.dt_consist.load_drug_graph(self.__targets__,limit2drugs)
    ...
    def find_targets(self):
        ...
        self.__targets__.update(self.__GVtargets())
       ...
       # Adding targets from disease model pathways
        ...
        if self.params['strict_mode']:
            ...
            regulators = disease_regulators.psobjs_with(only_with_values=self.params['target_types'])
            self.targets4strictmode.update(regulators)
    ...
    def load_target_partners(self):
        ...
        if receptor_dbids:
            ...
            ligands_oql = f'SELECT Relation WHERE objectType = DirectRegulation AND Effect = positive AND RelationNumberOfReferences > 5 AND \
...
NeighborOf upstream (SELECT Entity WHERE objectType = SmallMol) AND RelationNumberOfReferences > 50'
        ...
        self.partner2targets.remove_nodes_by_outdegree(max_degree=10,having_values=['SmallMol'])
        self.partner2targets.remove_nodes_by_outdegree(max_degree=10,only_with_prop=['Class'], having_values=['Ligand'])
        self.partner2targets.remove_nodes_by_indegree(max_degree=10,having_values=['SmallMol'])
    ...
    def set_target_disease_state(self):
        ...
        model_graph_dbid2uid = model_graph.dbid2uid()
        def __disease_state_from_model():
            ...
            activated_targets_dbid2uid = model_graph.dbid2uid(self._all_dbids(activ_targets_pd))
            inhibited_targets_dbid2uid = model_graph.dbid2uid(self._all_dbids(inhib_targets_pd))
            ...
            for idx in self.RefCountPandas.index:
                if self.RefCountPandas.at[idx,'State in Disease'] == 'unknown':
                    ...
                    for target_dbid in self.RefCountPandas.at[idx,self.__temp_id_col__]:
                        try:
                            target_uid = model_graph_dbid2uid[target_dbid]
                            activates_activated_targets,inhibits_activated_targets = model_graph.net_regulator_effect(target_uid,list(activated_targets_dbid2uid.values()))
                            activates_inhibited_targets,inhibits_inhibited_targets = model_graph.net_regulator_effect(target_uid,list(inhibited_targets_dbid2uid.values()))
    ...
    def score_target_semantics(self):
        ...
        if self.input_symptoms:
            ...
            linked_row_count = self.link2RefCountPandas(colname,self.input_symptoms)
        ...
        if self.input_clinpars:
            ...
            linked_row_count = self.link2RefCountPandas(colname,self.input_clinpars)
        ...
        if self.input_cellprocs:
            ...
            self.set_how2connect(['Regulation'],[],'',how2clone=REFERENCE_IDENTIFIERS)
            linked_row_count = self.link2RefCountPandas(colname,self.input_cellprocs)
        ...
        if hasattr(self, 'drugs_linked2disease'):
            ...
            was_linked,linked_entities,activ_targets_pd = self.link2concept(colname,self.drugs_linked2disease,activ_targets_pd)
            ...
            was_linked,linked_entities,inhib_targets_pd = self.link2concept(colname,self.drugs_linked2disease,inhib_targets_pd)
            print('Linked %d targets to drugs for "%s"' % (was_linked,t_n))
        ...
        if hasattr(self, 'disease_inducers'):
            ...
            was_linked,linked_entities,activ_targets_pd = self.link2concept(colname,self.disease_inducers,activ_targets_pd)
            ...
            was_linked,linked_entities,inhib_targets_pd = self.link2concept(colname,self.disease_inducers,inhib_targets_pd)
            print('Linked %d targets to inducers of "%s"' % (was_linked,t_n))
        ...
        if all_compounds:
            ...
            was_linked,linked_entities,unk_targets_pd = self.link2concept(colname,all_compounds,unk_targets_pd)
