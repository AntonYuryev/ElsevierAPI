...
from .ResnetAPISession import NO_REL_PROPERTIES,ONLY_REL_PROPERTIES,REFERENCE_IDENTIFIERS
...
class DiseaseTargets(SemanticSearch):
    ...
    def __init__(self, APIconfig, params=dict()):
        ...
        self.columns2drop.append('State in Disease')
        # if strict mode algorithm only ranks targets suggetsed in the literarure without predicting new targets
        # if not strict mode algorithm also predicts and rank additional targets based on target expression or genetic profiles in disease
        
    def set_input(self):
        ontology_graph = self.child_graph(self.param['disease'],['Name','Alias'])
    ...
    def input_names(self):
        prop2values = {'Name':self.param['disease'], 'Alias':self.param['disease']}
    ...
    def refcount_columns(self,counts_df=df()):
        ...
        return [col for col in d.columns if self._col_name_prefix in col] + [PATHWAY_REGULATOR_SCORE]

    def _get_report_name(self):
        rep_pred = 'suggested ' if self.param['strict_mode'] else 'suggested,predicted ' 
        return str(self.param['data_dir']+rep_pred+ 'targets for '+ ','.join(self.param['disease']))

    def _disease2str(self):
        if hasattr(self,'diseases'):
            if len(self.diseases) > 1:
                return str(len(self.diseases))+' types of '+','.join(self.param['disease'])
            else:
                return ','.join(self.param['disease'])
        else:
            return ','.join(self.param['disease'])

    def __target_ids(self):
        return self.targets4strictmode if self.param['strict_mode'] else self.target_ids

    def __target_types_str(self):
        return ','.join(self.param['target_types'])
    ...
    def __GVtargets(self):
        ...
        oql_query = 'SELECT Relation WHERE objectType = GeneticChange AND NeighborOf (SELECT Entity WHERE objectType = ({})) \
            AND NeighborOf ({})'
    ...
    def find_targets(self):
        ...
        self.target_ids = set(target_disease_graph.get_node_ids(self.param['target_types']))
        ...
        if self.param['strict_mode']:
            ...
            regulators_ids = disease_regulators.get_node_ids(self.param['target_types'])
            self.targets4strictmode.update(regulators_ids)

    def find_symptoms(self):
        if self.param['symptoms']:
            ...
            oql2select_symptom = OQL.get_childs(self.param['symptoms'],['Name'],include_parents=True)
    ...
    def find_clinpar(self):
        if self.param['clinical_parameters']:
            REQUEST_NAME = 'Find clnical parameters linked to {}'.format(self._disease2str())
            OQLquery = 'SELECT Relation WHERE objectType = FunctionalAssociation AND NeighborOf ({}) AND NeighborOf ({})'
            oql2select_clinpar = OQL.get_childs(self.param['clinical_parameters'],['Name'],include_parents=True)
    ...
    def find_cellproc(self):
        if self.param['processes']:
            REQUEST_NAME = 'Find cell processes linked to {}'.format(self._disease2str())
            OQLquery = 'SELECT Relation WHERE objectType = FunctionalAssociation AND NeighborOf ({}) AND NeighborOf ({})'
            oql2select_cellproc = OQL.get_childs(self.param['processes'],['Name'],include_parents=True)
    ...
    def load_target_partners(self):
        ...
        if receptor_ids:
            ...
            print('No Receptor Class entities were found among %s targets' % self.param['target_types'])
    ...
    def load_pathways(self):
        ...
        PSPathway objects from folders listed in self.param['pathway_folders']
        keeps only pathways listed self.param['pathways'] if latter is not empty 
    ...
    def init_semantic_search(self):
        ...
        self.RefCountPandas = self.load_pandas(target2score_df,prop_names_in_header=True,map2type=self.param['target_types'])
    ...
    def score_regulators(self):
        print('\n\nScoring regulators by distance to components of disease pathways')
    ...
    def score_target_semantics(self):
        ...
        if self.symptoms_ids:
            ...
            was_linked = self.link2RefCountPandas(colname,self.symptoms_ids)
            #linked_row_count = new_session.link2RefCountPandas(colname,self.symptoms_ids)
        ...
        if self.clinpar_ids:
            ...
            was_linked = self.link2RefCountPandas(colname,self.clinpar_ids)
            #linked_row_count = new_session.link2RefCountPandas(colname,self.clinpar_ids)
        ...
        if hasattr(self, 'drug_ids'):
            ...
            #new_session = self._clone_(to_retrieve=REFERENCE_IDENTIFIERS)
          ...
          #  t1 = Thread(target=self.link2concept, args=(colname,self.drug_ids,'',activ_targets_pd))
    ...
    def normalize_counts(self):
        copy_columns = ['Target partners']
        if hasattr(self,'__colnameGV__'):
            copy_columns.append(self.__colnameGV__)
