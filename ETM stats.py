...
def etm_hitcount(query, APIconfig:dict):
    ...
    return result['total-hits='] 


def download_json_etm(search_name, query, APIconfig:dict):
    print('Performing ETM search "%s"'%search_name)
    print ('Query: %s' % query)
    ETMurl = APIconfig['ETMURL']  #ETMurl = 'https://discover.elseviertextmining.com/api/'
    request_type = '/search/advanced'
    baseURL = ETMurl+request_type+'?'
    ...
    HitCount = etm_hitcount(query,APIconfig)
    ...
    dump_file.close()
    time.sleep(20) #file is not created fast enough for subsequent reading
...
def parse_abstacts(article:dict):
    ...
    for paragraph in abstract_secs:
        ...
        try:
            ...
            try:
                if isinstance(paragraph['sec'], dict):
                    sentences = [paragraph['sec']]
                else:
                    assert isinstance(paragraph['sec'], list)
                    sentences = paragraph['sec']

                abs_text = str()
                for s in sentences:
                    abs_text += s['addressOrAlternativesOrArray']['p']+' '
                abs_text = abs_text[:-1]
...
def report_hitcounts(etm_queries):
    with open('HitCounts stats.txt', 'w') as report:
        for search in etm_queries:
            dump_fname = make_json_dump_name(search[1])
            articles = json.load(open(dump_fname))
            report.write(search[1]+'\t'+str(len(articles)) +'\n')
...
if __name__ == "__main__":
    ...
    for search in etm_queries:
        ...
        print('Query "%s" found %d articles' %(query,HitCount))
